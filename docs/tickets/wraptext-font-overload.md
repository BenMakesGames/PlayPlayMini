...
- [ ] Add a `[Theory]` to `WordWrapTests.cs` (or a new sibling test method) constructing a multi-sheet `Font` from two `FontSheet`s with disjoint, contiguous char ranges (e.g., sheet A covers `' '`..`'@'` width 1, sheet B covers `'A'`..`'~'` width 2). Assert wrapping splits at expected boundaries given the heterogeneous widths.
- [ ] Add a sanity `[Theory]` row that calls the new `Font` overload with `new Font(FontSheet)` (single-sheet) and asserts the output matches the existing `FontSheet`-overload row's expected output for the same input — proves fast-path parity.
- [ ] Spot-check `BenMakesGames.PlayPlayMini.Performance\WrapText.cs` still compiles and produces a sensible benchmark number (no perf regression target — informational only).
- [ ] Inspect generated XML doc output (or hover in IDE) on the new overload to confirm tags render.
