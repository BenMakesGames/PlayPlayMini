    ...
    public class GraphicsManager: IServiceLoadContent, IServiceInitialize
    {
        ...
        public void LoadContent(GameStateManager gsm)
        {
            ...
            Pictures = pictures
                ...
                .ToDictionary(
                    meta => meta.Key,
                    meta => Content.Load<Texture2D>(meta.Path)
                )
            ;
            ...
            SpriteSheets = spriteSheets
                ...
                .ToDictionary(
                    meta => meta.Key,
                    meta => new SpriteSheet(Content.Load<Texture2D>(meta.Path), meta.Width, meta.Height)
                )
            ;
        ...
        )
        {
            ...
            {
                Pictures.Add(meta.Key, Content.Load<Texture2D>(meta.Path));
            }

            foreach(var meta in spriteSheets.Where(m => !m.PreLoaded))
            {
                SpriteSheets.Add(meta.Key, new SpriteSheet(Content.Load<Texture2D>(meta.Path), meta.Width, meta.Height));
            }

            Fonts = fonts.ToDictionary(
                meta => meta.Key,
                meta => new Font(Content.Load<Texture2D>(meta.Path), meta.Width, meta.Height)
            );
